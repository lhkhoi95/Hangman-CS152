...
@router.post("/")
async def retrieve_games(word_list_id: int, db: Session = Depends(get_db)):
    word_list_db = get_word_list_by_id(db, word_list_id)
...
async def mark_station_as_completed(station_id: int, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    background_tasks.add_task(prepare_next_route, station_db)

    return games_crud.mark_station_as_completed(db, station_id)


@router.post("/next-route")
async def retrieve_games_for_next_route(game_id: int, db: Session = Depends(get_db)):
    ...
    # Select words from the range of indices
    word_list_db = get_word_list_by_id(db, game_db.wordListId)
...
def prepare_next_route(station_db):
    try:
